...
#Step10: Create a new instance from your AMI image in the N. Virginia region:
aws ec2 run-instances \
    --image-id "$amiID" \
...
#Step11: Get Instance ID of the instance created in Step10:
...
#Step12: Wait untill EC2 launches (becomes running state):
aws ec2 wait instance-running \
    --instance-ids "$instanceID2"

#Step13: Get Public IP of instance:
publicIP=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=Assignment7task1" --query "Reservations[*].Instances[*].PublicIpAddress" --output=text)

#Step14: SSH to Instance created in Step10 and check the content of the index.html file (should be the same as in instance created in Step4):
ssh -o "StrictHostKeyChecking=no" -i /var/root/.ssh/"$keypair".pem ec2-user@"$publicIP" 'ls /usr/share/nginx/html' | grep 'alexabuy.jpg' > ~/check1.txt

#Step15: SSH to Instance created in Step10 and check if the alexabuy.jpg exists in the Nginx root directory:
ssh -o "StrictHostKeyChecking=no" -i /var/root/.ssh/"$keypair".pem ec2-user@"$publicIP" 'cat /usr/share/nginx/html/index.html' > ~/check2.txt

#Step16: Copy AMI image from the N. Virginia region to the Ohio region:
aws ec2 copy-image \
    --region "$regionOhio" \
    --name ami-name \
...
#Step 17: Set Ohio AWS Region (us-east-2):
...
#Step20: Get AMI image ID from AMI created in Step14:
amiID2=$(aws ec2 describe-images --filters "Name=name,Values=ami-name" --query 'Images[*].[ImageId]' --output text)

sleep 3

#Step21: Wait untill AMI launches (becomes available):
aws ec2 wait image-available \
    --filters "Name=name,Values=ami-name" --output text


#Step22: Create a new instance from AMI image created in Step14 (in the Ohio region):
aws ec2 run-instances \
    --image-id "$amiID2" \
...
#Step23: Get Instance ID of the nstance created in Step3:
instanceID3=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=Assignment7task2Ohio" --query Reservations[*].Instances[*].[InstanceId] --output text)

#Step24: Wait untill EC2 launches (becomes running state):
aws ec2 wait instance-running \
    --instance-ids "$instanceID3"


#Step25: Get Public IP of instance:
publicIP1=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=Assignment7task2Ohio" --query "Reservations[*].Instances[*].PublicIpAddress" --output=text)

#Step26: SSH to Ohio Server created in Step20:
ssh -o "StrictHostKeyChecking=no" -i /var/root/.ssh/"$keypairOhio".pem ec2-user@"$publicIP1" 'free -m' > ~/check3.txt

#Step27: Set Ohio AWS Region (us-east-2):
aws configure set region "$regionNVirginia"

#Step28: Change instance type to t2.micro:
    #stop
...
#Step29: Wait untill EC2 launches (becomes running state):
aws ec2 wait instance-running \
    --instance-ids "$instanceID"
sleep 2

#Step30: Get Public IP of instance:
publicIP2=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=Assignment7task1" --query "Reservations[*].Instances[*].PublicIpAddress" --output=text)

#Step31: SSH to Ohio Server created in Step20:
ssh -o "StrictHostKeyChecking=no" -i /var/root/.ssh/"$keypair".pem ec2-user@"$publicIP2" 'free -m' > ~/check4.txt
